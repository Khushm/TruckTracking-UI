...
function App() {
  ...
  const notopenNotificationWithIcon = () => {
    notification.open({
      message: "Success",
      // sudhis is the content of the notification. This is the content of the notification. This is the content of the notification.',
  ...
  const onSubmit = () => {
    if (
      ...
      for (let i = 0; i < camera.length; i++) {
        axios({
          method: "POST",
          url: `http://172.17.0.7:5001/trucktrack_api/get_data/`,
          data: {
          ...
          .then((res) => {
            ...
            const proc_data = [];
            arr.map((item) => {
              const filter_data = proc_data.filter(
                (e) => e._uuid == item._uuid
              );
  ...
  columns.push({
    ...
    render: (record) => {
      ...
      return <ImageComponent url={img1} objs={bbox} />;
...
>>>>>>> 89213769e57c5effac81441c86262c10c8dc0f5d
        if (val.length != 0) {
          return {
            children: (
              <Popover
          ...
          return {
            children: (
              <CloseSquareFilled style={{ color: "red", fontSize: "18px" }} />
            ),
            // <CloseOutlined  />,
          };
        }
  ...
  return (
    <div className="App">
      ...
      <Space direction="horizontal">
        ...
        <DatePicker allowClear={true} onChange={onChange} format={dateFormat} />
